		...
		<div class="row">
					...
					<div class="card-body">
						<table class="table table-bordered">
							...
							<?php 
								...
								$out = $conn->query("SELECT sum(available) as `out` FROM inventory where supply_id = ".$row['id']);
			...
			<div class="col-md-7">
				<div class="card">
					<div class="card-header">
						Supply In/Out List
						<button class="btn btn-primary btn-sm float-right" id="manage-supply">Add Supply</button>
					</div>
					<div class="card-body">
						<table class="table table-bordered">
							<thead>
							...
							<?php 
								...
								while($row=$inventory->fetch_assoc()):
									$sup_qty_arr[$row['id']] = $row['qty'];
									$sup_avl_arr[$row['id']] = $row['available'];
							...
							?>
								<tr>
									...
									<td class="text-right"><?php echo $sup_qty_arr[$row['id']]?></td>

									<td class="text-right"><?php echo $sup_avl_arr[$row['id']]?></td>
...
<script>
	...
	$('.edit_stock').click(function(){
		uni_modal("Manage Supply","manage_inv2.php?id="+$(this).attr('data-id'))
	})
